...
#reflections #list of canvas assingments

today = datetime.now(timezone.utc)
t=[]

for item in reflections:
    query_url=API_URL+"/courses/"+COURSE+"/assignments/"+item
    query=requests.request("GET",query_url, headers=headers)
    if (query.status_code==200):
        data=query.json()
        due_at=data['due_at']
        due_datetime=dateutil.parser.parse(due_at)
        t.append((today-due_datetime).days)
    else:
        print("Error:"+str(query.status_code))



delay=min([n for n in t if n>=0])
assignment_id=str(reflections[t.index(delay)])
...
if len(data_set)!=0:
    ...
    for question in range(questions):
        ...
        file_name=IMG_PATH+"reflection_wordcloud_"+str(question)+".png"
...
file_name=IMG_PATH+"reflection_submission_scores.png"
...
file_name=IMG_PATH+"reflection_activity_scores.png"
plt.savefig(file_name)
plt.close()
...
file_name=IMG_PATH+"reflection_times.png"
plt.savefig(file_name)
plt.close()
...
file_name=IMG_PATH+"reflection_submissions.png"
plt.savefig(file_name)
plt.close()
...
html+="<a href='"+IMG_PATH+"reflection_submissions.png'>"
html+="<img src='"+IMG_PATH+"reflection_submissions.png' class='rounded'>"
html+="</a>"
html+="</div>"
html+="</div>"
...
html+="<div class='text-center'>"
html+="<a href='"+IMG_PATH+"reflection_times.png'>"
html+="<img src='"+IMG_PATH+"reflection_times.png' class='rounded'>"
html+="</a>"
html+="</div>"
html+="</div>"
...
for question in range(questions):
    ...
    html+="<div class='text-center'>"
    html+="<a href='"+IMG_PATH+"reflection_wordcloud_"+str(question)+".png'"+">"
    html+="<img src='"+IMG_PATH+"reflection_wordcloud_"+str(question)+".png'"+" class='rounded'>"
    html+="</a>"
    html+="</div>"
    html+="</div>"
...
html+="<div class='text-center'>"
html+="<a href='"+IMG_PATH+"reflection_submission_scores.png'>"
html+="<img src='"+IMG_PATH+"reflection_submission_scores.png' class='rounded'>"
html+="</a>"
html+="</div>"
html+="</div>"
...
html+="<div class='text-center'>"
html+="<a href='"+IMG_PATH+"reflection_activity_scores.png'>"
html+="<img src='"+IMG_PATH+"reflection_activity_scores.png' class='rounded'>"
html+="</a>"
html+="</div>"
html+="</div>"
...
f = open('reflection_insight.html','w')
f.write(html)
f.close()
